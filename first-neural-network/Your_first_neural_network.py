...
class NeuralNetwork(object):
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            #X.shape (3,)
            #self.weights_input_to_hidden.shape (2,3)
            ...
            #self.weights_hidden_to_output.shape(2,1)
            ...
            #损失率 error_term 表示损失率
            output_error_term = np.array(1.0)
            #output_error_term.shape(1,)
#             hidden_error_term = np.dot(hidden_outputs,(1- hidden_outputs))
            hidden_error_term =  hidden_error*hidden_outputs*(1- hidden_outputs)
            #hidden_error_term.shape (1,)
            # Weight step (input to hidden)
#             print(np.dot(hidden_error,hidden_error_term).reshape(-1,1))
#             print(np.dot(X.reshape(-1,1),np.dot(hidden_error,hidden_error_term).reshape(-1,1).T))
            delta_weights_i_h += np.dot(X.reshape(-1,1),hidden_error_term.reshape(-1,1).T)
#             delta_weights_i_h += np.dot(X.reshpae(-1,1),,(hidden_error*hidden_error_term).reshape(-1,1))
            #delta_weights_i_h.shape(3,)
...
class TestMethods(unittest.TestCase):
    ...
    def test_run(self):
        ...
        network.weights_input_to_hidden = test_w_i_h.copy()
        network.weights_hidden_to_output = test_w_h_o.copy()
        # 注意：因为矩阵相乘的结果可能是一个矩阵，也可能一个值，这需要根据神经网络的结构来控制
...
预测效果不是特别理想。可能是在拟合的时候，没有做早期停止（early stop）
